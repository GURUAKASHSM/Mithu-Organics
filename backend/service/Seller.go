...
func Inventory(inventory models.Inventory) (string, error) {
	...
	sellerid, err := ExtractCustomerID(inventory.SellerName, "")
	if err != nil {
	...
	if cursor.RemainingBatchLength() == 0 {
		...
		err := config.Product_Collection.FindOne(context.TODO(), bson.M{"sellerid": inventory.SellerId}).Decode(&seller)
		if err != nil {
			return "Seller not found", err
		}
		inventory.SellerName = seller.Seller_Name
		inventory.ProductId = GenerateUniqueProductID()
		_, err = config.Product_Collection.InsertOne(context.Background(), inventory)
		if err != nil {
...
func UpdateProductBySeller(update models.UpdateProduct) (string,error) {
	id,err := ExtractCustomerID(update.Token,"")
	if err != nil{
		return "Login Expired",err
	}
	...
	err = config.Product_Collection.FindOne(context.Background(),filter).Decode(&inventory)
	if err != nil{
	...
	_, err = config.Product_Collection.UpdateOne(context.TODO(), filter, updatefilter, options)
	if err != nil {
...
func DeleteProductBySeller(delete models.DeleteBySeller) (string, error) {
	id, err := ExtractCustomerID(delete.Token, "")
	if err != nil {
		return "Login Expired", err
	}
	var inventory models.Inventory
...
func Orders(token models.Token) ([]models.AddOrder, string, error) {
	...
	cursor, err := config.Order_Collection.Find(context.Background(), filter)
	if err != nil {
...
func RegisterSeller(seller models.Seller) (string, error) {
	...
	for cursor.Next(context.Background()) {
		...
		_, err = config.Product_Collection.ReplaceOne(context.Background(), filter, seller)
		if err != nil {
...
func SellerDrashbordDetails(token models.Token) (models.DrashBoard, string, error) {
	...
	completedCount, err := config.Order_Collection.CountDocuments(context.Background(), map[string]interface{}{
	...
	notCompletedCount, err := config.Order_Collection.CountDocuments(context.Background(), map[string]interface{}{
		"status.confirmed": "completed",
		"sellerid":         id,
...
func GetCustromerOrderforSeller(details models.GetOrder) (*models.AddOrder, string, error) {
	...
	err = config.Order_Collection.FindOne(context.Background(), filter).Decode(&orderDetails)
	if err != nil {
...
func UpdateOrderTracking(details models.OrderTracking) (string, error) {
	...
	var order models.AddOrder
	filter := bson.M{"orderid": details.OrderID}
	err = config.Order_Collection.FindOne(context.Background(), filter).Decode(&order)
	if err != nil {
	...
	if details.Feild == "processing" && order.Status.Processing_Order == "pending" {
		...
		_, err = config.Order_Collection.UpdateOne(context.Background(), filter, update)
		if err != nil {
			return "Error in Updating", err
		}
		return "Updated Successfully", nil
