...
func Addtocart(addtocart models.Addtocart) (string, error) {
	id, err := ExtractCustomerID(addtocart.Token, "")
	if err != nil {
		log.Println(err)
	...
	err = config.Product_Collection.FindOne(context.Background(), inventoryfilter).Decode(&inventoryData)
	if err != nil {
		log.Println(err)
	...
	_, err = config.Product_Collection.UpdateOne(context.Background(), inventoryfilter, update)
	if err != nil {
		return "Error In Updating", err
	}
...
func FetchInventoryData(search models.Search) (*models.Inventory, error) {
	...
	err := config.Product_Collection.FindOne(context.Background(), filter).Decode(&data)
	if err != nil {
		log.Println(err)
		return nil, err
	}
...
func UpdateCart(cart models.Addcart) (bool, string) {
	Id, err := ExtractCustomerID(cart.CustomerId, "")
	if err != nil {
		log.Println(err)
...
func DeleteProduct(delete models.DeleteProduct) bool {
	customerid, err := ExtractCustomerID(delete.Token, "")
	if err != nil {
		log.Println(err)
		return false
	}
	...
	var data models.Inventory
	err = config.Product_Collection.FindOne(context.Background(), filter3).Decode(&data)
	if err != nil {
		log.Println(err)
		return false
	}
	...
	_, err = config.Product_Collection.UpdateOne(context.Background(), filter3, update1)
	if err != nil {
		log.Println(err)
		return false
	}
...
func CustomerOrders(Buynow models.BuyNow) (string, error) {
	for _, value := range Buynow.ItemsToBuy {
		...
		_, err := config.Product_Collection.InsertOne(context.Background(), order)
		if err != nil {
			log.Println(err)
			return "Error in inserting", err
		}
...
func DeleteOrder(delete models.DeleteOrder) (string, error) {
	...
	err := config.Product_Collection.FindOne(context.Background(), filter).Decode(&order)
	if err != nil {
...
func TotalAmount(id string) float64 {
	...
	err = config.Product_Collection.FindOne(context.Background(), bson.M{}).Decode(&currentValues)
	if err != nil {
		log.Println(err)
		return 0
	}
	...
	_, err = config.Product_Collection.UpdateOne(
		context.Background(),
		bson.M{},
		bson.M{
			"$set": bson.M{
...
func AddUserAddress(address models.AddAddress) (string, error) {
	id, err := ExtractCustomerID(address.Token, "")
	if err != nil {
		return "Login Expired", err
	}
	filter := bson.M{"customerid": id}
