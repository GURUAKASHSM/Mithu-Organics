...
// Admin Login
func AdminLoginCheck(login *models.AdminData) (string, bool,error) {
	...
	if err != nil {
		return "Wrong Username or Password", false,err
	}
	if correctdata.WrongInput == 4 {
		return "Maximum no of Try Reached",false, err
	}
	if correctdata.IP_Address != login.IP_Address {
		return "It's not your Valid IP",false, nil
	}
	if HashAdminPassword(correctdata.Password) != login.Password {
		...
		return "Wrong Username or Password",false, nil
	}
